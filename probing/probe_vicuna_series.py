...
def collect_vlm_text(args, device) -> Tuple[torch.Tensor, torch.Tensor]:
    try:
        processor = AutoProcessor.from_pretrained(args.vlm_name, use_fast=True)
    except Exception:
        processor = AutoProcessor.from_pretrained(args.vlm_name)

    if device.type == "cuda" and not args.force_fp32:
        dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    else:
        dtype = torch.float32

    model = AutoModelForVision2Seq.from_pretrained(
        args.vlm_name, torch_dtype=dtype, low_cpu_mem_usage=True
    ).eval().to(device)
    ...
    for i, (img, text) in enumerate(tqdm(prompts, desc=f"VLM collect [{args.cond}]")):
        try:
            if img is not None:
                # 이미지가 있으면(black) vision 토큰 개수가 텍스트 내 placeholder 개수와 일치해야 하므로,
                # collect_vlm_img_phd와 동일하게 <image> placeholder를 텍스트 앞에 붙인다.
...
def main():
    ...
    ap.add_argument("--llm_name", type=str, default="lmsys/vicuna-7b-v1.5")
    ap.add_argument("--vlm_name", type=str, default="llava-hf/llava-1.5-7b-hf")
