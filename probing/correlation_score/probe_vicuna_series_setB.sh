#!/usr/bin/env bash
# probe_vicuna_series_setB.sh
### Vicuna / Mistral / LLaVA series probing — Set B (HaluEval context, VLM w/ black image) ###
set -euo pipefail
...
MAX_SAMPLES=0
OUT_ROOT="/root/Desktop/workspace/miso/faithful-probing/probing_code/script_camera_ready/correlation_score/outputs/vicuna_cross_dataset_probing_set_b_cv5"
mkdir -p "${OUT_ROOT}"
...
OUT_VLM_BLACK="${OUT_ROOT}/llava1.5"
...
OUT_VLM_BLACK="${OUT_ROOT}/llavanxt"
mkdir -p "${OUT_VLM_BLACK}"
python3 "${PY}" \
  --mode vlm \
  --vlm_name "${VLM_NAME2}" \
