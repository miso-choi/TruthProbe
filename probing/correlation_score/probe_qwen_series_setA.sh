#!/usr/bin/env bash
# probe_qwen_series_setA.sh
### Qwen2.5 / Mistral series probing — Set A (HaluEval context, VLM text_only) ###
set -euo pipefail
...
MAX_SAMPLES=0
OUT_ROOT="/root/Desktop/workspace/miso/faithful-probing/probing_code/script_camera_ready/correlation_score/outputs/qwen_cross_dataset_probing_set_a_cv5"
mkdir -p "${OUT_ROOT}"
...
OUT_VLM_TEXT="${OUT_ROOT}/qwen2.5_vl_instruct"
...
OUT_VLM_TEXT="${OUT_ROOT}/qwen2.5_vl_omni"
mkdir -p "${OUT_VLM_TEXT}"
python3 "${PY}" \
  --mode vlm \
  --vlm_name "${VLM_NAME2}" \
